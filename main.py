...
if sys.platform=='win32':
    ...
    # 获取系统 DPI 缩放比例（Windows 默认 96 DPI = 100%）
    try:
        _hdc = ctypes.windll.user32.GetDC(0)
        _logpixelsy = ctypes.windll.gdi32.GetDeviceCaps(_hdc, 90)  # LOGPIXELSY
        ctypes.windll.user32.ReleaseDC(0, _hdc)
        _dpi_scale = round(_logpixelsy / 96.0, 2)
    except Exception:
        _dpi_scale = 1.0
...
class ExcelBridgeApp(ctk.CTk):
        ...
        for n in names:
            ...
            if os.path.exists(p):
                try:
                    ...
                    img=Image.open(p)
                    self._icons[n]=ctk.CTkImage(img,img,size=(16,16))
                except Exception: self._icons[n]=None
            else: self._icons[n]=None

    # ── 布局入口 ──

    def _build(self):
        saved_font = getattr(self, '_font_var', None)
        saved_val = saved_font.get() if saved_font else self._saved_font_size
    ...
    def _fs(self, size):
        ...
        return round(size * base / 12)

    def _on_font_change(self, choice):
        self._saved_font_size = choice
        self._build()

    def _config_dir(self):
        """配置文件目录：exe 所在目录（打包后）或脚本目录（开发时）"""
        if getattr(sys, 'frozen', False):
    ...
    def _save_config(self):
        try:
            ...
            cfg = {
                ...
                'skip_hidden': self._skip_hidden.get() if hasattr(self, '_skip_hidden') else False,
            }
            path = os.path.join(self._config_dir(), '.excelbridge_config.json')
    ...
    def _load_config(self):
        try:
            ...
            pass
        return {'font_size': '12', 'skip_hidden': False}
    ...
    def _build_left(self):
        ...
        self._btn(bb, "加载规则", None, 28, '#F3F4F6', C.TEXT_HELP, 'E5E7EB',
                  lambda: self._act("加载规则"), icon='import').grid(row=1, column=0, sticky='ew')
    ...
    def _on_toggle_skip_hidden(self):
        """勾选/取消跳过隐藏时，刷新当前源文件预览"""
        self._save_config()
        if hasattr(self, '_current_src_path') and self._current_src_path:
            self._on_file_select(self._current_src_path)
    ...
    def _on_file_select(self, filepath):
        ...
        try:
            ...
            self._src_tree.destroy()
            self._src_tree = self._make_table(parent, True, max(ncols, 1))
            self._src_tree.grid(row=0, column=0, sticky='nswe')
    ...
    def _browse_target(self):
        ...
        if path:
            ...
            try:
                ...
                self._tgt_tree = self._make_table(parent, False, max(ncols, 1))
                self._tgt_tree.grid(row=0, column=0, sticky='nswe')
                for i, row in enumerate(rows):
                    vals = [str(c) if c is not None else '' for c in row]
                    tag = 'even.X' if i % 2 == 0 else 'odd.X'
    ...
    def _make_table(self, parent, is_source, col_count=10):
        """创建美化的 Treeview（样式已由 _setup_styles 初始化）"""
        cols = tuple(chr(65 + i) for i in range(col_count))
    ...
    def _build_right(self):
        ...
        self._mini_log.tag_config('success', foreground=C.SUCCESS)
        self._mini_log.tag_config('warn', foreground=C.MATCH_ORANGE)
        self._mini_log.tag_config('error', foreground=C.ERROR)
    ...
    def _btn(self, parent, text, width, height, fg, text_color, hover,
        ...
        if icon and self._icn(icon):
            kwargs['image'] = self._icn(icon)
    ...
    def _act(self, msg, level=None):
        """统一操作入口：打印 + 记录日志 + 更新 mini 日志。

        level: 'info' (默认), 'success' (✅), 'warn' (⚠), 'error' (❌)
        """
        import datetime
        # 自动检测日志级别：关键词匹配
        if level is None:
            if any(w in msg for w in ('失败', '错误', '异常', '无法')):
                level = 'error'
            elif any(w in msg for w in ('完成', '成功', '✅')):
                level = 'success'
            elif any(w in msg for w in ('未找到', '跳过', '警告', '不存在')):
                level = 'warn'
            else:
                level = 'info'

        # 级别前缀
        prefixes = {'success': '✅ ', 'warn': '⚠ ', 'error': '❌ ', 'info': ''}
    ...
    def _open_log_window(self):
        ...
        # 日志颜色标签
        log_box.tag_config('info', foreground=C.TEXT_HELP)
        log_box.tag_config('success', foreground=C.SUCCESS)
        log_box.tag_config('warn', foreground=C.MATCH_ORANGE)
        log_box.tag_config('error', foreground=C.ERROR)
    ...
    def _open_about(self):
        ...
        sf = ctk.CTkScrollableFrame(win, fg_color='transparent',
                                     scrollbar_button_color=C.BORDER,
                                     scrollbar_button_hover_color=C.TEXT_MUTED)
        sf.pack(fill='both', expand=True, padx=16, pady=(8, 8))
        ...
        features = [
            ("核心功能", [
                ...
                "可视化点击选列映射",
                "隐藏行列智能过滤",
                "字号调节（10-18）· 高 DPI 自适应",
                "日志着色：成功 / 警告 / 错误",
                "xlsx / xls 双格式支持",
            ]),
            ("技术信息", [
                "Python 3.8.10 · CustomTkinter",
                "openpyxl · xlrd · xlwt · Pillow",
                "Windows 7 / 10 / 11 (64位)",
            ]),
        ]
        ...
        # 关闭按钮（固定不滚动）
        ctk.CTkButton(win, text="关闭", width=100, height=34,
                      fg_color=C.PRIMARY, text_color='white',
                      hover_color=C.PRIMARY_HOVER, corner_radius=8,
                      command=win.destroy).pack(pady=(0, 16))
